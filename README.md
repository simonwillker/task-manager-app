...
Playwright による E2E テスト（107 ケース。サーバー版 84 / 静的版 23）を用意しています。
...
テスト対象: 認証 / メールアドレスの確認 / パスワード再設定 / 回数制限 / 初期状態 / タスクの追加 / 完了状態の切り替え / 編集 / フィルタ / 削除 / 選んでまとめて削除 / 書き出し・読み込み / 静的版（GitHub Pages）/ データベースへの保存（リロード・別端末・ユーザー間の分離・別タブ同期・localStorage からの引き継ぎ） / ヘルスチェック / XSS 対策 / パストラバーサル対策 / レスポンシブ / JS エラー検知
