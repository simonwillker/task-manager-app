...
- 完了・未完了の切り替え
- **タスクの編集（内容と期日を後から変更）** — 削除と違い、管理者でなくても合言葉なしで行えます
- フィルタ表示（すべて / 未完了 / 完了済み）
- **タスクの削除（管理者のみ・完了済みのみ・合言葉が必要）** — 下の「タスクの削除」を参照
- **選んでまとめて削除**（「選択」→ 消したいものにチェック → まとめて削除。同じ3条件が必要）
- 完了済みタスクの一括削除（同じ3条件が必要）
- タスク件数の表示（総数・未完了数）
- データベースへの保存（ユーザーごとに分離され、複数端末で共有されます）
- 同じブラウザの別タブへの即時反映、別の端末で変更した内容は画面に戻ったときに反映
- **タスクの書き出し・読み込み**（ファイル保存とコピー貼り付けの両方。2つの版の間でタスクを移せます）
- 以前のバージョンで `localStorage` に保存していたタスクを、初回ログイン時にアカウントへ自動で引き継ぎ

## タスクを別の版へ移す

静的版（GitHub Pages）とサーバー版（Fly.io）は**ドメインが違うため、タスクは自動では移りません**。
`localStorage` はドメインごとに分かれており、自動の引き継ぎは同じドメイン内でしか働かないためです。

移すときは画面下の「タスクを書き出す」「タスクを読み込む」を使います。

1. 移す元で **タスクを書き出す** → 「ファイルに保存」か「コピー」
2. 移す先で **タスクを読み込む** → ファイルを選ぶか、本文を貼り付けて「読み込む」

読み込みは**追加**です。今あるタスクは消えません。同じものを2回読み込めば2件になります。
書き出した本文のほか、タスクの配列だけの JSON も受け付けます（古い版の `localStorage` の形）。
一度に読み込めるのは 1000 件までです。
...
| `PATCH` | `/api/tasks/:id` | 内容・完了状態・期日の変更 `{ text?, completed?, dueDate? }`（`dueDate: null` で期日を外す。3つとも無ければ `400`） |
| `DELETE` | `/api/tasks/:id` | タスク削除 `{ password }`（管理者・完了済みのみ） |
| `POST` | `/api/tasks/bulk-delete` | 選んだタスクをまとめて削除 `{ ids: [...], password }`（管理者・完了済みのみ。1 回 1000 件まで） |
...
- 後から「編集」で付け外しできます（API では `PATCH /api/tasks/:id`）

## タスクの編集

一覧の「編集」ボタンから、**内容と期日をまとめて書き換え**られます。期日欄を空にすると期日が外れます。

削除と違って**管理者でなくても、合言葉なしで**行えます。書き換えは元に戻せる操作で、削除のように取り返しがつかないわけではないためです（自分のタスクだけが対象なのは削除と同じで、他人のタスクは `404` になります）。
...
「選択」を押すと選択モードに入り、完了済みのタスクにチェックを付けてまとめて消せます。条件は 1 件ずつの削除とまったく同じです。

- 未完了のタスクはチェックを付けられません（理由が出ます）。
- **未完了が 1 つでも混ざっていると、1 件も消しません**（`409` `task_not_completed`）。一部だけ消えるほうが分かりにくいためです。
- 別の端末で既に消えていた id は飛ばします（結果が同じなので失敗にしません）。
- 選択モードのあいだは、タスクの文字を押しても完了状態は変わらず、選択の切り替えになります。
- 「表示中の完了済みを選ぶ」は、**絞り込みで見えているぶんだけ**を選びます（見えていないものが消えると分からないため）。

**画面の動き**

- 管理者以外には削除ボタンも「選択」も「完了済みを削除」ボタンも表示されません。
...
Playwright による E2E テスト（111 ケース。サーバー版 84 / 静的版 27）を用意しています。
...
テスト対象: 認証 / メールアドレスの確認 / パスワード再設定 / 回数制限 / 初期状態 / タスクの追加 / 完了状態の切り替え / 編集 / フィルタ / 削除 / 選んでまとめて削除 / 書き出し・読み込み / 静的版（GitHub Pages）/ 画面幅ごとのレイアウト / データベースへの保存（リロード・別端末・ユーザー間の分離・別タブ同期・localStorage からの引き継ぎ） / ヘルスチェック / XSS 対策 / パストラバーサル対策 / レスポンシブ / JS エラー検知
